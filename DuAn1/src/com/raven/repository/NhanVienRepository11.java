...
import com.raven.viewmodel.NhanVienVM;

/**
 *
 * @author PH22349
 */
public class NhanVienRepository11  {
    ...
    public boolean add(NhanVien nv) {
      ...
      public List<MaNhanVienVM> getNV() throws Exception{
        String query = "SELECT maNV FROM [dbo].[NHANVIEN]";
        try ( Connection con = DBConnect.getConnection();  PreparedStatement ps = con.prepareStatement(query);) {
            ResultSet rs = ps.executeQuery();
            List<MaNhanVienVM> listMNV = new ArrayList<>();
            while (rs.next()) {
                MaNhanVienVM mnv = new MaNhanVienVM(rs.getString("maNV"));
                listMNV.add(mnv);
            }
            return listMNV;
...
//    public List<MaNhanVienVM> getOneNV(String maNv) {
//        String query = "SELECT maNV,hoTenNv FROM [dbo].[NHANVIEN] WHERE maNV = ? ";
//
//        try ( Connection con = DBConnect.getConnection();  PreparedStatement ps = con.prepareStatement(query);) {
//            List<MaNhanVienVM> listnv = new ArrayList<>();
//            ps.setObject(1, maNv);
//            ResultSet rs = ps.executeQuery();
//            while (rs.next()) {
//                MaNhanVienVM nv = new MaNhanVienVM(rs.getString(1), rs.getString(2));
//                listnv.add(nv);
//            }
//            return listnv;
//        } catch (Exception e) {
...
//    }
    ...
    public static void main(String[] args) throws Exception {
        List<MaNhanVienVM> lisst = new NhanVienRepository11().getNV();
        for (MaNhanVienVM x : lisst) {
